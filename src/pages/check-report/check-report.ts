...
import { IonicPage, NavController, NavParams,AlertController,Platform, ToastController,Events } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-check-report',
  templateUrl: 'check-report.html',
})
export class CheckReportPage {
  item: any;
  ...
  taskName: string;
  taskDefKey:string;
  flowFlag:string = 'lims_b_test_report';// 流程标识
  ...
  constructor(public navCtrl: NavController, 
    ...
    if(localStorage.getItem("serverIP")){
      this.downPdfUrl ="http://"+localStorage.getItem("serverIP")+"/LIMS/m/repo/limsBTestReport/downloadReport?reportId="+ this.item.id;
    } else {
      this.downPdfUrl ="http://47.94.236.108:18080/LIMS/m/repo/limsBTestReport/downloadReport?reportId="+ this.item.id;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad CheckReportPage');
  }
