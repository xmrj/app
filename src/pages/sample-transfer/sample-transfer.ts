...
@IonicPage()
@Component({
  selector: 'page-sample-transfer',
  templateUrl: 'sample-transfer.html',
})
export class SampleTransferPage {
  item: any;
  ...
  taskName: string;
  taskDefKey:string;
  flowFlag:string = 'lims_b_sample_transfer';// 流程标识
  ...
  constructor(public navCtrl: NavController, 
    ...
    if(localStorage.getItem("serverIP")){
      this.downPdfUrl ="http://"+localStorage.getItem("serverIP")+"/LIMS/m/samp/limsBSampleTransferHead/downLoadSampleTransfer?examineId="+ this.item.id;
    } else {
      this.downPdfUrl ="http://47.94.236.108:18080/LIMS/m/samp/limsBSampleTransferHead/downLoadSampleTransfer?examineId="+ this.item.id;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad SampleTransferPage');
  }
