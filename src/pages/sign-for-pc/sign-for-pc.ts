...
import { IonicPage, NavController,Navbar, NavParams,ToastController,AlertController,Events } from 'ionic-angular';
...
/**
 * Generated class for the SignForPcPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-sign-for-pc',
  templateUrl: 'sign-for-pc.html',
})
export class SignForPcPage {
   ...
   drawCancel() {
    ...
    upload() {
      if(typeof(this.signatureImage) == "undefined"||this.signatureImage.length==0){
        ...
        this.imageService.upload(params,"appr/limsBApproveManage/uploadSignaturePC").subscribe((res:any)=>{
          ...
          this.navCtrl.pop().then(() => {
            // 发布 bevents事件
            this.events.publish('signEventOver', 'OK2');
          });
  ...
  ionViewDidEnter() {
    ...
    me.navBar.backButtonClick = () => {
      this.navCtrl.pop().then(() => {
        // 发布 bevents事件
        this.events.publish('signEventOver', '-1');
      });
    };
  }
