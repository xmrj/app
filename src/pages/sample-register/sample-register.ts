...
import { IonicPage, NavController,Platform, NavParams,ToastController,AlertController,Events } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-sample-register',
  templateUrl: 'sample-register.html',
...
export class SampleRegisterPage {
  ...
  downPdfUrl :string ;
  //="http://192.168.1.139:18081/LIMS/m/samp/limsBSamplingResultExamine/downloadSampExaminePDF";
  opinion:string; // 审批意见
  taskName: string;
  taskDefKey:string;
  flowFlag:string = 'lims_b_sample_register';// 流程标识
  ...
  constructor(
    ...
    if(localStorage.getItem("serverIP")){
      this.downPdfUrl ="http://"+localStorage.getItem("serverIP")+"/LIMS/m/samp/limsBSamplingResultExamine/downloadSampExaminePDF?examineId="+ this.item.id;
    } else {
      this.downPdfUrl ="http://47.94.236.108:18080/LIMS/m/samp/limsBSamplingResultExamine/downloadSampExaminePDF?examineId="+ this.item.id;
    }
    //this.downPdfUrl =this.downPdfUrl+"?examineId="+ this.item.id;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad SampleRegisterPage');
  }
  ...
  pdf(item){
    ...
    if(localStorage.getItem("serverIP")){
      this.downPdfUrl ="http://"+localStorage.getItem("serverIP")+"/LIMS/m/pdf/yulan?path="+encodeURI(encodeURI(item.filePath));
    }else{
      this.downPdfUrl="http://47.94.236.108:18080/LIMS/m/pdf/yulan?path="+encodeURI(encodeURI(item.filePath));
    }
